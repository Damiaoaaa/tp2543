...
<head>
  ...
  <title>My car part store : Orders</title>
  ...
  <?php include_once 'nav_bar.php'; ?>
    ...
    <form action="orders.php" method="post">
      <div class="form-group">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-9">
          <input name="date" type="text" class="form-control" id="orderdate" placeholder="Order Date" value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>">
          </div>
        </div>
        <div class="form-group">
          <label for="staffid" class="col-sm-3 control-label">Staff</label>
          <div class="col-sm-9">
      <select name="sid" class="form-control" id="customergender"required>
        <?php
        try {
      ...
      <div class="form-group">
          ...
          <div class="col-sm-9">
      <select name="cid" class="form-control" id="customergender"required>
      <?php
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
   ...
   <div class="row">
    ...
    try {
      $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
      $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $sql = "SELECT * FROM tbl_orders_a178816_fpt, tbl_staffs_a178816_fpt, tbl_customers_a178816_fpt WHERE ";
      $sql = $sql."tbl_orders_a178816_fpt.fld_staff_id = tbl_staffs_a178816_fpt.fld_staff_id and ";
      $sql = $sql."tbl_orders_a178816_fpt.fld_customer_id = tbl_customers_a178816_fpt.fld_customer_id ORDER BY tbl_orders_a178816_fpt.fld_order_date DESC LIMIT {$start_from}, {$per_page}";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a178816_fpt");
            $stmt->execute();
            $result = $stmt->fetchAll();
